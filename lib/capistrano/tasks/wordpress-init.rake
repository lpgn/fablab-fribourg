namespace :wp do
  ...
  # Create dump and shared/config directories
  desc 'Initialize Wordpress Project'
  task :init do
    on roles(:web) do
      execute "mkdir #{fetch(:home)}/dump/#{fetch(:application)}"
      execute "mkdir #{fetch(:home)}/dump/#{fetch(:application)}/local"
      execute "mkdir #{fetch(:home)}/dump/#{fetch(:application)}/remote"
      execute "mkdir #{shared_path}/config"
    end
  end

  # Write and push config file
  task :config do
    config = File.open(fetch(:configfile), "w+" )
    config.puts("<?php
/**
...
define('DB_NAME', '#{fetch(:database)[fetch(:stage)][:remote][:name]}');

/** MySQL database username */
define('DB_USER', '#{fetch(:database)[fetch(:stage)][:remote][:user]}');

/** MySQL database password */
define('DB_PASSWORD', '#{fetch(:database)[fetch(:stage)][:remote][:password]}');

/** MySQL hostname */
define('DB_HOST', '#{fetch(:database)[fetch(:stage)][:remote][:host]}');
...
/** Site URL */
define('WP_HOME','#{fetch(:siteurl)}');
define('WP_SITEURL','#{fetch(:siteurl)}/wordpress');
...
")
    config.close
    system "scp #{fetch(:configfile)} #{fetch(:user)}@#{fetch(:host)}:#{shared_path}/config/"
    system "rm #{fetch(:configfile)}"
  end
end